...
import (
	...
	//"net/http/httputil" //https://rominirani.com/golang-tip-capturing-http-client-requests-incoming-and-outgoing-ef7fcdf87113
...
func NewOAuthProxy(opts *options.Options, validator func(string) bool) (*OAuthProxy, error) {
	...
	logger.LogTrace("OAuthProxy configured for %s Client ID: %s", opts.GetProvider().Data().ProviderName, opts.ClientID)
	refresh := "disabled"
	if opts.Cookie.Refresh != time.Duration(0) {
		refresh = fmt.Sprintf("after %s", opts.Cookie.Refresh)
	}

	logger.LogTrace("Cookie settings: name:%s secure(https):%v httponly:%v expiry:%s domains:%s path:%s samesite:%s refresh:%s", opts.Cookie.Name, opts.Cookie.Secure, opts.Cookie.HTTPOnly, opts.Cookie.Expire, strings.Join(opts.Cookie.Domains, ","), opts.Cookie.Path, opts.Cookie.SameSite, refresh)
...
func (p *OAuthProxy) makeCookie(req *http.Request, name string, value string, expiration time.Duration, now time.Time) *http.Cookie {
	...
	if cookieDomain != "" {
		...
		if !strings.HasSuffix(domain, cookieDomain) {
			logger.Errorf("Warning: request host is %q but using configured cookie domain of %q %+v", domain, cookieDomain, req)
		}
	}
...
func (p *OAuthProxy) OAuthStart(rw http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		logger.Errorf("Error obtaining nonce: %v %+v", err, req)
		p.ErrorPage(rw, req, http.StatusInternalServerError, err.Error())
		return
	}
...
func (p *OAuthProxy) OAuthCallback(rw http.ResponseWriter, req *http.Request) {
	...
	if p.Validator(session.Email) && authorized {
		...
		logger.LogTrace("OAuth User Login: %s connected on %s", session.Email, time.Now())
		http.Redirect(rw, req, redirect, http.StatusFound)
	} else {
...
func (p *OAuthProxy) redeemCode(req *http.Request) (*sessionsapi.SessionState, error) {
	...
	//logger.Printf("requestDump: %s", requestDump)
